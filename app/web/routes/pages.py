...
async def delete_todo(request: Request, todo_id: int, csrf_token: str = Form(None), db: AsyncSession = Depends(get_db), user: UserModel = Depends(get_user_from_cookie)):
    if user is None:
...
async def complete_todo(request: Request, todo_id: int, csrf_token: str = Form(None), db: AsyncSession = Depends(get_db), user: UserModel = Depends(get_user_from_cookie)):
    if user is None:
        return RedirectResponse(url="/auth/login", status_code=status.HTTP_303_SEE_OTHER)
    # CSRF validation
    if not validate_csrf_token(request, csrf_token):
...
async def subscribe(request: Request, user: UserModel = Depends(get_user_from_cookie)):
    ...
    async with httpx.AsyncClient(base_url="http://localhost:8000") as client:
        try:
            ...
            set_flash(request, "Failed to initiate subscription. Please try again later.", category="danger")
            return RedirectResponse(url="/dashboard", status_code=status.HTTP_303_SEE_OTHER)
...
async def create_todo(request: Request, title: str = Form(...), description: str = Form(None), csrf_token: str = Form(None), db: AsyncSession = Depends(get_db), user: UserModel = Depends(get_user_from_cookie)):
    ...
    if getattr(user, "subscription_status", None) != "active":
        ...
        if todo_count >= 10:
            set_flash(request, "Free users are limited to 10 todos. Subscribe for unlimited todos.", category="danger")
            return RedirectResponse(url="/dashboard", status_code=status.HTTP_303_SEE_OTHER)
...
@router.post("/account")
async def account_post(request: Request, display_mode: str = Form(None), items_per_page: int = Form(None), show_email: bool = Form(False), profile_public: bool = Form(False), db: AsyncSession = Depends(get_db), user: UserModel = Depends(get_user_from_cookie), csrf_token: str = Form(None), redis=Depends(get_redis)):
    if user is None:
        return RedirectResponse(url="/auth/login", status_code=status.HTTP_303_SEE_OTHER)
...
async def friends_page(request: Request, db: AsyncSession = Depends(get_db), user: UserModel = Depends(get_user_from_cookie), csrf: str = Depends(get_csrf_token)):
    if user is None:
        return RedirectResponse(url="/auth/login", status_code=status.HTTP_303_SEE_OTHER)
    ...
    formatted_friends = []
    for f in friends:
        is_online = (f.last_seen and (datetime.now(__import__('datetime').timezone.utc) - f.last_seen).total_seconds() < 300)
    ...
    return templates.TemplateResponse(request, "pages/friends.html", {
        "request": request,
...
async def chat_page(friend_id: int, request: Request, db: AsyncSession = Depends(get_db), user: UserModel = Depends(get_user_from_cookie)):
    if user is None:
        return RedirectResponse(url="/auth/login", status_code=status.HTTP_303_SEE_OTHER)
    ...
    stmt = select(FriendshipModel).where(
        and_(
            ...
            or_(
                and_(FriendshipModel.user_id == user.id, FriendshipModel.friend_id == friend_id),
                and_(FriendshipModel.user_id == friend_id, FriendshipModel.friend_id == user.id)
            )
        )
    )
    res = await db.execute(stmt)
    if not res.scalar_one_or_none():
         set_flash(request, "You can only chat with accepted friends.", category="danger")
...
async def admin_cookies(request: Request, db: AsyncSession = Depends(get_db), user: UserModel = Depends(get_user_from_cookie), csrf: str = Depends(get_csrf_token), flash: str = Depends(get_and_pop_flash)):
    ...
    if q:
        users = (await db.execute(select(UserModel).where((UserModel.email.ilike(f"%{q}%")) | (UserModel.role.ilike(f"%{q}%"))).order_by(UserModel.created_at.desc()).limit(200))).scalars().all()
    else:
        users = (await db.execute(select(UserModel).order_by(UserModel.created_at.desc()).limit(200))).scalars().all()
    return templates.TemplateResponse(request, "pages/admin_cookies.html", {"request": request, "users": users})


@router.post('/admin/cookies/clear')
async def admin_cookies_clear(request: Request, user_id: int = Form(...), csrf_token: str = Form(None), db: AsyncSession = Depends(get_db), user: UserModel = Depends(get_user_from_cookie)):
    ...
    new_prefs = {k: v for k, v in (prefs.items() if isinstance(prefs, dict) else []) if k != 'cookie_consent'}
...
async def admin_cookies_bulk_clear(request: Request, user_ids: list[int] = Form(...), csrf_token: str = Form(None), db: AsyncSession = Depends(get_db), user: UserModel = Depends(get_user_from_cookie)):
    ...
    for t in targets:
        ...
        if 'cookie_consent' in prefs:
            ...
            # audit row
            a = AuditLog(actor_id=user.id, target_user_id=t.id, action='clear_cookie_consent', details=dumps({'prev': prefs.get('cookie_consent')}))
            db.add(a)
            modified += 1
            # invalidate cache
            try:
...
async def admin_cookies_audit(request: Request, db: AsyncSession = Depends(get_db), user: UserModel = Depends(get_user_from_cookie), csrf: str = Depends(get_csrf_token), flash: str = Depends(get_and_pop_flash)):
    if user is None or user.role != 'admin':
        set_flash(request, "Access denied. Admin privileges required.")
        return RedirectResponse(url="/dashboard", status_code=status.HTTP_303_SEE_OTHER)

    from app.infrastructure.database.models.audit_log_model import AuditLog
    audits = (await db.execute(select(AuditLog).order_by(AuditLog.created_at.desc()).limit(200))).scalars().all()
    return templates.TemplateResponse(request, "pages/admin_cookies_audit.html", {"request": request, "audits": audits})


@router.get('/admin/cookies/export')
async def admin_cookies_export(request: Request, db: AsyncSession = Depends(get_db), user: UserModel = Depends(get_user_from_cookie)):
    ...
    q = request.query_params.get('q')
    if q:
        res = await db.execute(select(UserModel).where((UserModel.email.ilike(f"%{q}%")) | (UserModel.role.ilike(f"%{q}%"))).order_by(UserModel.created_at.desc()).limit(1000))
    else:
        res = await db.execute(select(UserModel).order_by(UserModel.created_at.desc()).limit(1000))
    ...
    for u in users:
        ...
        writer.writerow([u.id, u.email, prefs.get('cookie_consent') if isinstance(prefs, dict) else ''])
